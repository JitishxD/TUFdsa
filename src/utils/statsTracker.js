 ...
 * @returns {string}
 */
export const getTodayDateString = (now = Date.now()) =>
  timestampToDateString(now);
 ...
 * @returns {string}
 */
export const getYesterdayDateString = (now = Date.now()) =>
  timestampToDateString(now - MS_PER_DAY);
...
export const calculateStreakFromLog = (activityLog, options = {}) => {
  ...
  const longestStreak = Math.max(
    streak,
    calculateLongestStreakFromLog(activityLog),
  );
...
export const getCountForDate = (solveHistory, dateString) => {
  if (!solveHistory) return 0;
  return Object.values(solveHistory).filter(
    (ts) => timestampToDateString(ts) === dateString,
  ).length;
};
...
export const calculateStatsWithOptions = (
  ...
  const streakResult = calculateStreak(mergedHistory, {
    now,
    allowGracePeriod,
  });
...
export const validateSolveHistory = (solveHistory) => {
  ...
  for (const [problemId, timestamp] of Object.entries(solveHistory)) {
    ...
    if (timestamp > Date.now() + MS_PER_DAY) {
      issues.push(
        `Future timestamp for problem ${problemId}: ${new Date(timestamp)}`,
      );
    }
  }
