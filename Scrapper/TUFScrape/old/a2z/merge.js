...
const gfgMap = new Map(
    dataGfg.map(item => [item.problemName, item])
);
const codeMap = new Map(
    dataCode.map(item => [item.problemName, item])
);
...
const mergedData = data2.map(item => {
    ...
    if (matchGfg || matchCode) {
        ...
        if (matchCode && Array.isArray(matchCode.codestudio) && !item.codestudioLink) {
        ...
        if ((matchGfg?.gfgLink?.length ?? 0) > 1 ||
            ...
            if ((matchGfg?.gfgLink?.length ?? 0) > 1) {
                console.log(`> gfgLink count: ${matchGfg.gfgLink.length}, ${matchGfg.gfgLink}`);
            }
            if ((matchCode?.codestudio?.length ?? 0) > 1) {
                console.log(`> codestudio count: ${matchCode.codestudio.length}, ${matchCode.codestudio}`);
            }
        }
